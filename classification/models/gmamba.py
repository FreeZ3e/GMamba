...
class _layer:
    def __init__(
        ...
        self.branch_mask = ASR(self.d_inner, 2)
        self.GC = Grapher(in_channels=self.d_inner, kernel_size=9, dilation=dilation, conv='mr', act='gelu',
                                 norm='batch', epsilon=0.2, r=reduce_ratio, n=HW, drop_path=dpr)
        self.channel_mask = ACR(self.d_inner)
    ...
    def forwardv2(self, x: torch.Tensor, **kwargs):
        ...
        y = self.forward_core(x).unsqueeze(-1)
        y1 = (x + self.GC(x)).unsqueeze(-1)

        scales = torch.cat((y, y1), -1)
        y = torch.sum(scales * mask, dim=-1)
        y = self.ACR(y)
...
class layer(nn.Module, _layer):
    def __init__(
        ...
        self.__init__(**kwargs, dilation=dilation, reduce_ratio=reduce_ratio, HW=HW, dpr=dpr)
    ...
    def __init__(
        ...
        for i_layer in range(self.num_layers):
            ...
            self.layers.append(self._make_layer(
                ...
                _SS2D=layer,
                # =================
                reduce_ratio=reduce_ratios[i_layer],
                HW=HWs[i_layer]
            ))
